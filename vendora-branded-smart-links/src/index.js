...
const SHORT_DOMAIN_MODES = new Set(["vendora", "ultra", "country"]);
...
function isShortLinkHost(host, env) {
  const allowed = [shortOrigin(env), ultraShortOrigin(env)]
    .map((origin) => {
      try {
    ...
    .filter(Boolean);
  return allowed.includes(host) || host.startsWith("go.") || host.startsWith("g.");
}
