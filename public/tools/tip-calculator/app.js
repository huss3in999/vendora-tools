...
(function () {
  ...
  var quickTipButtons = Array.prototype.slice.call(document.querySelectorAll(".quick-tip-button"));
  var resetButton = document.getElementById("resetButton");

  var tipAmountOutput = document.getElementById("tipAmountOutput");
  var totalAmountOutput = document.getElementById("totalAmountOutput");
  var totalBillOutput = document.getElementById("totalBillOutput");
  var perPersonOutput = document.getElementById("perPersonOutput");
  ...
  function updateQuickButtons() {
    var currentTip = String(getNumber(tipPercentInput));
    quickTipButtons.forEach(function (button) {
      button.classList.toggle("is-active", button.getAttribute("data-tip") === currentTip);
    });
  }

  function calculate() {
    var billAmount = getNumber(billAmountInput);
    var tipPercent = getNumber(tipPercentInput);
    var peopleCount = Math.max(1, Math.round(getNumber(peopleCountInput) || 1));
    var tipAmount = billAmount * (tipPercent / 100);
    var totalBill = billAmount + tipAmount;
    var perPerson = totalBill / peopleCount;

    tipAmountOutput.textContent = formatMoney(tipAmount);
    totalAmountOutput.textContent = formatMoney(totalBill);
    totalBillOutput.textContent = formatMoney(totalBill);
    perPersonOutput.textContent = formatMoney(perPerson);
    peopleOutput.textContent = String(peopleCount);

    if (!billAmountInput.value) {
      statusText.textContent = "Enter your bill and tip percentage to see the total instantly.";
    } else if (peopleCount > 1) {
      statusText.textContent = "The bill is split across " + peopleCount + " people including tip.";
    } else {
      statusText.textContent = "Your total includes the selected tip amount.";
    }

    if (peopleCountInput.value !== String(peopleCount)) {
  ...
  [currencyInput, billAmountInput, tipPercentInput, peopleCountInput].forEach(function (input) {
  ...
  quickTipButtons.forEach(function (button) {
    button.addEventListener("click", function () {
      tipPercentInput.value = button.getAttribute("data-tip");
      calculate();
    });
  });
