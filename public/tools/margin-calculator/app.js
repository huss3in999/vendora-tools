...
(function () {
  ...
  function calculate() {
    ...
    if (!costAmountInput.value || !sellingPriceInput.value) {
      ...
      statusText.textContent = "Enter cost and selling price to calculate your margin instantly.";
      return;
    }

    if (profit < 0) {
      pricingStatusOutput.textContent = "Loss";
      statusText.textContent = "The selling price is below cost, so this item is losing money.";
    } else if (margin >= 60) {
      pricingStatusOutput.textContent = "Strong";
      statusText.textContent = "This pricing shows a strong profit margin and healthy markup.";
    } else if (margin >= 30) {
      pricingStatusOutput.textContent = "Balanced";
      statusText.textContent = "This pricing shows a workable margin for many business situations.";
    } else {
      pricingStatusOutput.textContent = "Low";
      statusText.textContent = "This pricing has a low margin, so it may need review.";
    }
  }
