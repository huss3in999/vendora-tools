...
function corsHeaders8(request) {
  const origin = request.headers.get("origin") || "";
  console.log("Incoming CORS Origin:", origin, "Has in ALLOWED_ORIGINS:", ALLOWED_ORIGINS8.has(origin));
  return {
  ...
  try {
    ...
    if (!object) {
      ...
      if (staticRes.ok) {
        ...
        return json9(staticData, { headers });
      }
      return json9({ settings: {}, products: [], categories: [] }, { headers });
    }
    const data = await object.json();
    return json9(data, { headers });
  } catch (err) {
    console.error(err);
    return json9({ settings: {}, products: [], categories: [] }, { headers });
  }
...
async function handleSaveCatalog(request, env, headers) {
  ...
  if (contentLength > MAX_JSON_BYTES2) {
    return json9({ ok: false, error: "Catalog size exceeds 512KB limit" }, { status: 413, headers });
  }
  let payload;
  try {
    ...
    return json9({ ok: false, error: "Invalid JSON payload" }, { status: 400, headers });
  }
  if (!payload || typeof payload !== "object") {
    return json9({ ok: false, error: "Catalog must be a JSON object" }, { status: 400, headers });
  }
  const key = "maroc-market/data/catalog.json";
...
// .wrangler/tmp/bundle-hyRrl2/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-hyRrl2/middleware-loader.entry.ts
