...
# Enter a PIN different from the Owner and Worker PINs
...
- Accountant: `CHANGE_ME_ACCOUNTANT`
...
## Owner Purchase and Accountant Upgrade

The Owner can record personally paid business purchases without changing the Worker cash ledger. Detailed purchases contain one or more product lines; quick purchases contain a total and general category/description. Product categories, products, and suppliers are Owner-managed master data. Historical purchases retain snapshot names when master records are later renamed or hidden.

The Accountant is a separate PIN role with backend-enforced read-only access to the current Worker cash summary, Worker expenses, Owner purchases, combined expense reports, purchase details/receipts, and filtered CSV exports.
...
Apply it to production only during an approved deployment window:
...
Configure receipt images optionally with Cloudflare R2. Create a bucket, then add this binding before deployment:
...
Without `RECEIPTS`, all purchase functions remain available but receipt upload/view returns a configuration message. Accepted receipt types are JPEG, PNG, and WebP up to 8 MB. Receipt object keys are stored only in D1 and image access goes through authenticated Owner/Accountant routes.
