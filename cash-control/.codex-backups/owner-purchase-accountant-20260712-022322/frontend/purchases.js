...
import { formatBD, showToast, showLoading, navigate, backHeader, screenLayout } from './app.js?v=19';
...
function bindForm() {
  ...
  document.getElementById('purchase-items')?.addEventListener('input',e=>{const row=e.target.closest('[data-index]');if(!row)return;const item=items[Number(row.dataset.index)];item[e.target.dataset.field]=e.target.value;if(e.target.dataset.field==='quantity'||e.target.dataset.field==='unit_price'){if(item.quantity&&item.unit_price)item.line_total=(Number(item.quantity)*Number(item.unit_price)).toFixed(3);}renderItems();});
