...
async function getPinSettings(env) {
  ...
  return {
    ...
    accountant_pin: dbAccountant || env.ACCOUNTANT_PIN || '2222',
...
async function listMasterData(env, resource, params) {
  ...
  const sql = `SELECT * FROM ${config.table}${where.length ? ` WHERE ${where.join(' AND ')}` : ''} ORDER BY ${resource === 'products' ? 'is_favourite DESC, last_used_at DESC,' : ''} display_order, name`;
  const rows = await env.DB.prepare(sql).bind(...binds).all();
...
async function getAccountantDashboard(env,params) {
  const today=await businessDateStr(env); const dates=reportDates(params,today); const includeTest=params.get('show_test')==='1';
  const cash=await getWorkerDashboard(env,{role:'accountant'});
  const testSql=includeTest?'1=1':'is_test=0'; const dateSql=`${dates.from?' AND business_date>=?':''}${dates.to?' AND business_date<=?':''}`; const binds=[...(dates.from?[dates.from]:[]),...(dates.to?[dates.to]:[])];
...
async function getUnifiedExpenses(env,params) {
  const today=await businessDateStr(env); const dates=reportDates(params,today); const includeTest=params.get('show_test')==='1'; const source=params.get('source'); const rows=[];
  ...
  if (source!=='owner') {
    const binds=[]; let where="type='expense' AND status='active'"+(includeTest?'':" AND is_test=0");
  ...
  if(source!=='worker') {
    const search=new URLSearchParams(params); if(dates.from)search.set('date_from',dates.from);if(dates.to)search.set('date_to',dates.to);search.set('status','active');if(includeTest)search.set('show_test','1');
...
async function exportAccountantCsv(env,params){
  ...
  if(kind==='items'){
    ...
    for(const expense of expenses.expenses){const detail=await getOwnerPurchase(env,expense.id);for(const item of detail.items)records.push([expense.date,expense.id,expense.supplier,item.product_name_snapshot,item.category_name_snapshot,item.quantity,item.unit,item.unit_price,item.line_total,expense.is_test]);}
  }else{
