...
async function login(request, env) {
  ...
  if (requestedRole === "owner") {
    ...
    const token2 = await timed("tokenCreate", () => createSessionToken({ userId: 1, role: "owner", name: "Owner" }, env), timings);
...
async function getExpenseOptions(env) {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      const options = parsed.map((v) => String(v).trim()).filter(Boolean).slice(0, 100);
...
__name(getExpenseOptions, "getExpenseOptions");
...
__name(updateExpenseOptions, "updateExpenseOptions");
...
__name(bahrainNowParts, "bahrainNowParts");
function dateStringFromParts({ year, month, day }) {
  return `${year}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
}
__name(dateStringFromParts, "dateStringFromParts");
...
__name(previousLocalDate, "previousLocalDate");
...
__name(businessDateStr, "businessDateStr");
function todayStr() {
...
__name(getTodaySums, "getTodaySums");
...
async function getWorkerDashboard(env, session) {
  ...
  const expected = opening + todaySums.cash_sale + todaySums.cash_added_by_owner - todaySums.expense - todaySums.cash_taken_by_owner + todaySums.correction;
...
__name(listActivity, "listActivity");
...
__name(listClosings, "listClosings");
...
__name(voidClosing, "voidClosing");
...
__name(deleteClosing, "deleteClosing");
...
// .wrangler/tmp/bundle-XqwEKp/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-XqwEKp/middleware-loader.entry.ts
