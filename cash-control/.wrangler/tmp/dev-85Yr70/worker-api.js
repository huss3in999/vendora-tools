...
async function login(request, env) {
  ...
  if (requestedRole === "worker" && !pins.worker_login_enabled) {
    throw httpError(403, "Worker login is disabled. Contact owner.");
  }
  ...
  if (pins.owner_pin === pins.worker_pin && pin === pins.owner_pin) {
    if (requestedRole === "worker") {
      if (!pins.worker_login_enabled) throw httpError(403, "Worker login is disabled");
...
async function getPinSettings(env) {
  ...
  const workerEnabled = await getSetting(env, "worker_login_enabled");
  return {
    owner_pin: dbOwner || env.OWNER_PIN || "1111",
    worker_pin: dbWorker || env.WORKER_PIN || "1111",
    worker_login_enabled: workerEnabled !== "0"
  };
}
...
async function getSettings(env, session = null) {
  ...
  const result = {
    test_mode: testRow === "1",
    worker_login_enabled: pins.worker_login_enabled,
    pins_configured: !!await getSetting(env, "owner_pin") || !!await getSetting(env, "worker_pin")
  };
  if (session?.role === "owner") {
    result.worker_access = pins.worker_login_enabled ? "enabled" : "disabled";
  }
  return result;
}
...
async function updateSettings(request, env, session) {
  ...
  if (typeof body.worker_login_enabled === "boolean") {
    await setSetting(env, "worker_login_enabled", body.worker_login_enabled ? "1" : "0");
  }
...
// .wrangler/tmp/bundle-6dximC/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6dximC/middleware-loader.entry.ts
